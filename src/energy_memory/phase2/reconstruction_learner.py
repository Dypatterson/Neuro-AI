"""Reconstruction-loss codebook learning for FHRR substrates.

Stores windows in a Hopfield memory with distractors, retrieves each
window using its own full encoding as the cue, then decodes *every*
position.  The codebook is updated to minimize reconstruction error
across all positions — giving a dense learning signal per window
instead of the sparse one-masked-position signal of error-driven
learning.

This directly attacks the pattern-separability bottleneck: codebook
vectors are pushed to make windows distinguishable in HD space.
"""
...
class ReconstructionLearner:
    ...
    def train(
        ...
        """Train codebook via full-window reconstruction.

        Stores *landscape_windows* in a Hopfield memory, then for each
        probe window: encodes it fully, retrieves from memory, decodes
        every position, and buffers failures.  Yields diagnostics after
        each consolidation event.
        """
        ...
        for window in probe_windows:
            if any(t == self.vocab.unk_id for t in window):
                continue
            ...
            for pos_idx in range(len(window)):
                target_id = window[pos_idx]
                if target_id in (self.vocab.unk_id, self.vocab.mask_id):
                    continue
    ...
    def _consolidate(self) -> dict:
        ...
        diagnostics = {
            ...
            "failure_rate": (
                self._total_failures / max(1, self._total_positions)
            ),
        }
        self._buffer.clear()
        return diagnostics
