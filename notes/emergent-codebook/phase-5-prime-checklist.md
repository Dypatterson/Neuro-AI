...
| D2 | Skewed/natural co-occurrence condition if cheap | partial | Report 075 records skewed hard-cell and candidate-grid diagnostics; Reports 089-093 cover `repo_sample_natural` source co-occurrence, source-frequency aliasing, and a low-frequency cleaned protocol |
...
| G4 | Controls E1-E6 run on the same test set | partial | Reports 075-090 controls are matched where available; Report 091 cross-checks residual analysis against the same Report 090 test set; Report 093 runs cleaned matched controls on the same selected query plan; full matrix still open |
| G5 | Report explicitly says no graduation claim unless all gates pass | done | Reports 069-093 explicitly preserve no-graduation boundary |
