...
- **A1 fix design pre-commitment** (the report-047 discovery-channel duplicate-atoms failure on the A+B+step3 substrate): **closed 2026-05-20** at design ([note](../notes/2026-05-20-discovery-channel-r-ema-init-dynamic-form.md)); A1 (substrate-derived `r_ema` initial condition at add-time, replacing the implicit `r_ema = 0` constant) recommended; anti-homunculus reviewer PASS. W1 (discovery-channel gate), W2 (diversified schema-store selection / MMR / DPP), W3 (scheduled strength rebalancing) explicitly rejected as wrong-shape. Six pre-committed falsification criteria. **Sequenced ahead of Path 3 / β** — A1 implementation is the next "separate decision" gate; β remains the deferred successor design.
