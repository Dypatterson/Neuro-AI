...
- **Active deliverable:** **Report 115** + the next-instrument decision **n=30 vs Frame B slope**. Gate 0 ran (n=10 wikitext): gauge **confirmed**, verdict **`G0->weak`** (DiD `+0.019` CI `[-0.121,+0.159]`, 6/10 positive, per-seed σ≈0.19 — underpowered, not a redesign trigger). Stage-1 variance decomp: σ is ~92% corpus-window-draw, ~3.8% codebook → recommended next is **window-averaging the Frame A endpoint DiD first** (keeps the level headline, no new mechanism); the slope's σ(β) is still untested.
...
- **2026-05-29** — Stage-1 variance decomposition: per-seed DiD σ is ~92% corpus-window-draw, ~3.8% codebook — corrects the 05-28 "codebook×corpus ⇒ slope is the fix" *motivation* (the "window-dominance kills the slope" read was an over-read; the slope removes *any* per-seed level shift). Slope reframe conditionally-approved but σ(β) untested; recommended next = window-averaging the Frame A endpoint DiD. [review](brainstorm-workspace/2026-05-29-frameb-grill/06-stage1-review-findings.md) · [archive](notes/status-log/2026-05.md)
...
- **2026-05-27** — Γ1 implementation **landed**, three pre-code gates green (14 new tests + 109/109 regression); n=10 headline gate authorized. [archive](notes/status-log/2026-05.md)
