...
- **Active blockers:**
  1. Phase 5 path decision still open. **Single-source pickup for next session: [notes/notes/2026-05-24-phase5-session-close-and-next-moves.md](notes/notes/2026-05-24-phase5-session-close-and-next-moves.md)** — synthesizes the three remaining options (Path D / M2 = Tier-2 training-time intervention via EqProp + role-shuffled negatives + DSM warm-start [RECOMMENDED]; Path D / M1 = P1+D3+P3 stack [more ambitious, longer prerequisite chain]; Path B' as closure paper). Reading STATUS.md + that note is sufficient to continue without re-reading the brainstorm or spike notes. Original Paths A / B' / C are exhausted: Path A is closure-paper-only per the 2026-05-23 SNR-invariance walk-back; Path C is exhaustively characterized (one-hot form Report 061 is arbitration-shape, asymmetric-field form Report 063 is null and worsens monotonically with strength); D1 / D3 / E1 smokes (Reports 062 + 063) closed the three retrieval-mechanism families (storage rule, branch coupling, landscape reshaping). **Path D is the only path not ruled out at smoke scale.** No commitment in-session without user.
