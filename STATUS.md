...
- **Active blockers:**
  1. Phase 5 path decision still open. Original three options were Path A (capacity-proportional scale-down probe) / Path B' (close + pivot to surprise/PE-driven replay) / Path C (continue log-prior diagnostics). The 2026-05-24 rescue brainstorm + D1/D3/S1 Tier-0 spike results add **Path D = Tier-2 training-time intervention** (M2 EqProp+role-shuffled-negatives+DSM warm-start, or M1 P1+D3+P3 stack). D1 (pseudo-inverse storage swap), D3 (cross-K softmax), and E1 (centered log-prior field) have all returned null at smoke scale; storage-rule, branch-coupling, and landscape-reshaping retrieval families are now all ruled out. Path C is exhaustively characterized. **Path D is the only path not ruled out at smoke scale.** No commitment in-session without user.
