...
- **A1**: substrate-aware r_ema initialization for new atoms (smallest patch).
  **Design note written + anti-homunculus PASS** —
  ...
  Implementation landed at [f037e48](https://github.com/Dypatterson/Neuro-AI/commit/f037e48);
  1-seed pilot ([report 048](reports/048_phase5_a1_pilot_seed17.md))
  FAILS criteria #1 and #2 at n=1 (partial effect, but the first-added
  discovery atom still dominates). **Root cause: the Gram-row RMS
  proxy under-measures sparse duplicates.** A1 itself is correct in
  shape; the measurement is too weak. Three new sketched directions
  in report 048: A1' (max-over-others RMS, smallest patch),
  A1'' (streaming low-rank SVD per the original design note),
  A1''' (path-3 β as the alternative axis). No implementation
  commitment yet.
