...
- **Last verified result:** [Report 064](reports/064_phase5_m1_retrieval_smoke_cross_seed_null.md) — Path D / **M1 retrieval-side variant** (P1+D3+P3 stack with geometric codebook-prior row weights, the only non-degenerate M1 weight protocol per the P1 probe) is **null at smoke scale on all 3 substrate seeds**. `hit_role = 0.000` across 90 cues; `ΔE = −0.32 ± 0.01` across seeds (58× below floor in the wrong direction); `rank_role` median 453–540 of ~1024 patterns. Cross-seed variance is *smaller* than within-seed variance — not seed-17 idiosyncratic. **Phase 5 not graduated.**
- **Active blockers:**
  1. **Path D split, retrieval-side ruled out.** Four retrieval-mechanism families (D1 storage, D3 branch-coupling, E1 landscape-reshape, **M1 role-energy stack**) have now returned `hit_role = 0.000` on the existing FHRR substrate (Reports 062, 063, 064). The retrieval-time intervention space is exhausted at smoke scale. **Path D / M2** (training-time intervention: EqProp + role-shuffled-negatives + DSM warm-start) is the only Phase 5 path not yet smoked.
  2. **STATUS blocker #3 retired.** The "rerun a provenance-bearing substrate, then run the n≥10 control matrix" gate from the 2026-05-24 M1 implementation banner no longer makes sense as written: the retrain would not change the substrate-level diagnosis that the four-family null implicates, and M1's other weight variant (encoder-count) is degenerate per the P1 geometric probe regardless of provenance.
  3. **Architectural commit decision pending.** Three legitimate options: (a) commit to M2 implementation on a new branch; (b) accept Phase 5 closure / pivot per the 2026-05-23 SNR walk-back and the Path B' framing; (c) try the dual-code GHRR P1 variant before M2 (algebra change rather than training change). Decision pre-empts further smoke runs.
