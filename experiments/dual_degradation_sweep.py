"""Dual degradation sweep for coupled settling.

This corrupts both the content cue and the temporal cue. It is the first stress
test that resembles live use, where neither cue is guaranteed to be an exact
stored vector.
...
CONTENT_MODES = ["exact_stair", "noisy_stair", "family_blend", "wrong_ladder", "noise_only"]
TEMPORAL_MODES = ["exact_pair", "single_neighbor", "neighbor_plus_noise", "wrong_pair", "noise_only"]
TEMPORAL_BETAS = [0.5, 1, 2, 4, 8, 16, 32, 64, 100]


def make_content_query(substrate: FHRR, vectors: Dict[str, Vector], mode: str) -> Vector:
    if mode == "exact_stair":
        return vectors["stair"]
    if mode == "noisy_stair":
        return substrate.perturb(vectors["stair"], noise=0.35)
    if mode == "family_blend":
        return substrate.bundle([vectors["stair"], vectors["ladder"], vectors["ramp"]])
    if mode == "wrong_ladder":
        return vectors["ladder"]
    if mode == "noise_only":
    ...
    for content_mode in CONTENT_MODES:
        for temporal_mode in TEMPORAL_MODES:
            for temporal_beta in TEMPORAL_BETAS:
                ...
                content_query = make_content_query(substrate, vectors, content_mode)
                temporal_query = make_temporal_query(substrate, vectors, temporal_mode)
                ...
                recall = temporal_association_score(expected, result.temporal_items, k=k)
...
def summarize(rows: List[Dict[str, float | int | str]]) -> str:
    lines = ["# MVP 0.6 Dual-Degradation Sweep", ""]
    lines.append("## Summary By Cue Pair")
    lines.append("")
    lines.append("| Content cue | Temporal cue | First committed beta | First correct beta | Regimes observed |")
    lines.append("|---|---|---:|---:|---|")
    for content_mode in CONTENT_MODES:
        for temporal_mode in TEMPORAL_MODES:
            subset = [
                row
                for row in rows
                if row["content_mode"] == content_mode and row["temporal_mode"] == temporal_mode
            ]
            committed = [row for row in subset if row["regime"] == "committed"]
            correct = [
                row for row in subset if float(row["recall_at_4"]) >= 0.75 and row["top_label"] == "stair"
            ]
            ...
            regimes = ", ".join(sorted({str(row["regime"]) for row in subset}))
            lines.append(
                f"| {content_mode} | {temporal_mode} | "
                f"{first_committed if first_committed is not None else 'none'} | "
                f"{first_correct if first_correct is not None else 'none'} | {regimes} |"
            )

    lines.append("")
    lines.append("## Aggregate Regime Counts")
    counts: Dict[str, int] = {}
    for row in rows:
    ...
    for row in rows:
        lines.append(
            "| {content_mode} | {temporal_mode} | {temporal_beta} | {top_label} | {recall_at_4:.3f} | {final_top_weight:.3f} | {final_entropy:.3f} | {iterations} | {regime} |".format(
                **row
            )
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    parser.add_argument("--csv", default="reports/006_dual_degradation_sweep.csv")
    parser.add_argument("--report", default="reports/006_dual_degradation_sweep.md")
