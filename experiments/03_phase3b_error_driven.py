"""Phase 3b: Error-driven contrastive codebook refinement.

Loads the Hebbian-trained codebook from Phase 3a, applies error-driven
contrastive updates via masked-token retrieval failures, then evaluates
all three codebooks (random, Hebbian, Hebbian+error-driven) side-by-side.
"""
...
# ------------------------------------------------------------------
# Evaluation (identical to Phase 3a)
# ------------------------------------------------------------------
...
def write_comparison_report(
    ...
    lines.append("## Error-Driven Training Log")
    lines.append("")
    lines.append(
        "| Consolidation | Buffer | Pulled | Pushed | Mean Q | "
        "Total Retr | Total Fail | Fail Rate |"
    )
    lines.append("|---:|---:|---:|---:|---:|---:|---:|---:|")
    for entry in training_log:
        lines.append(
            ...
            f"| {entry['mean_quality']:.4f} | {entry['total_retrievals']} "
    ...
    lines.append(
        ...
        "| Random | Hebbian | Error-Driven | Bigram |"
    )
    lines.append("|---|---|---:|---:|---|---:|---:|---:|---:|---:|---:|")
    ...
    ed_rows = sorted(all_codebook_rows.get("error_driven", []), key=_sort_key)
    for ed_row in ed_rows:
        key = _row_key(ed_row)
        ...
        e_acc = ed_row["accuracy"]
        lines.append(
            "| {obj} | {ret} | {w} | {mc} | {mp} | {ls} | {beta:g} "
            "| {r:.3f} | {h:.3f} | {e:.3f} | {bg:.3f} |".format(
                obj=ed_row["objective"],
                ret=ed_row["retrieval_condition"],
                w=ed_row["window_size"],
                mc=ed_row["mask_count"] if ed_row["mask_count"] is not None else "-",
                mp=ed_row["mask_position"] or "-",
                ls=ed_row["landscape_size"],
                beta=ed_row["beta"],
                r=r_acc,
                h=h_acc,
                e=e_acc,
                bg=ed_row["bigram_accuracy"],
            )
        )
    ...
    for objective in ("masked_token", "next_token"):
        ...
        bg_rows = [
            r for r in all_codebook_rows.get("error_driven", [])
            if r["retrieval_condition"] == "generalization"
            and r["objective"] == objective
        ]
        bg_mean = (
        ...
        parts = [f"**{objective}**:"]
        for label in ("random", "hebbian", "error_driven"):
            if label in accs:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase 3b: Error-driven contrastive codebook refinement",
    )
    ...
    parser.add_argument("--epochs", type=int, default=3,
                        help="Number of error-driven passes over probe windows")
    ...
    parser.add_argument("--output-dir", default="reports/phase3b_error_driven")
    ...
    print(
        f"error-driven training: {args.train_landscape_size} landscape, "
    ...
    error_driven_codebook = learner.codebook
    save_codebook(error_driven_codebook, output_dir / "phase3b_codebook_error_driven.pt")
    ...
    save_vocabulary(vocab, output_dir / "phase3b_vocab.json")
    ...
    print("evaluating error-driven codebook...", flush=True)
    ed_rows = evaluate_codebook("error_driven", codebook=error_driven_codebook, **eval_kwargs)

    all_codebook_rows = {
        "random": random_rows,
        "hebbian": hebbian_rows,
        "error_driven": ed_rows,
    }
    ...
    config = {
        ...
        "consolidation_k": args.consolidation_k,
        "quality_threshold": args.quality_threshold,
        "train_mask_count": args.mask_count,
        "train_mask_position": args.mask_position,
    ...
    report_path = output_dir / "03_phase3b_error_driven.md"
    csv_path = output_dir / "03_phase3b_error_driven.csv"
